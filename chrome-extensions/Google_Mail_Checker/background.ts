...
var loggedInImage:HTMLElement = document.getElementById('logged_in');
var canvasContext:CanvasRenderingContext2D = canvas.getContext('2d');
var pollIntervalMin:number = 5;  // 5 minutes
var pollIntervalMax:number = 60;  // 1 hour
var requestTimeout:number = 1000 * 2;  // 2 seconds
...
interface hasInstanceId extends Storage{
      instanceId?:string
}

function getInstanceId():string {
  if (!localStorage.hasOwnProperty("instanceId"))
  var hasID:hasInstanceId=localStorage
  hasID.instanceId = 'gmc' + parseInt(String(Date.now() * Math.random()), 10);
  return hasID.instanceId;
}
